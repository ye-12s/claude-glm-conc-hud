...
```
 目录 | git 分支状态 | 模型 | 上下文 | GLM 并发 | 时间
```

并发段按阈值上色（默认 `≥9 红(满) / ≥7 黄 / 其余青`），可按你的账户上限自行修改。

上下文段（参考 claude-hud 的做法）读当前会话的 transcript，取**最近一次 assistant 响应**的 `usage`，按 `input + cache_creation + cache_read + output` 算出占上下文窗口的比例——注意 `cache_read` 是缓存命中的历史 prompt，同样占窗口，必须计入，否则会严重低估。用一条 **8 格进度条**（四舍五入到整格，避免低占比 1/8 块在深色背景下出现黑缝）+ `用量/总量` 展示，形如 `███░░░░░ 387.1k/1M`；按 `≥80% 红 / ≥50% 黄 / 其余绿` 上色，红了就该 `/compact` 了。

**窗口总量按当前模型动态识别，不写死**：优先解析 model id / `display_name` / `ANTHROPIC_*_MODEL` 里的 `[1m]` / `[200k]` / `[128k]` 后缀（Claude Code 用它标注上下文窗口变体，如 `glm-5.2[1M]` → 1,000,000）；匹配不到再用 `fmt_ctx()` 里的已知模型前缀表，最后兜底 `200k`。所以同一个脚本在 1M / 200k / 128k 模型下都能给出正确的百分比。
...
| 上下文窗口总量 | `hud-with-conc.sh` 里 `fmt_ctx()` 的 `ctx_window()`：解析 `[1m]/[200k]/[128k]` 后缀 + 模型前缀表（加新模型在这里加） |
| 上下文阈值 / 颜色 | `hud-with-conc.sh` 里 `fmt_ctx()` 的 `-ge` 判断（`≥80 红 / ≥50 黄 / 其余绿`） |
...
- `hud-with-conc.sh` — 状态栏组装（目录 / git / 模型 / 上下文 / 并发 / 时间）。
