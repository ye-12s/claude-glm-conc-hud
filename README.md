...
上下文段（参考 claude-hud 的做法）读当前会话的 transcript，取**最近一次 assistant 响应**的 `usage`，按 `input + cache_creation + cache_read + output` 算出占上下文窗口的比例——注意 `cache_read` 是缓存命中的历史 prompt，同样占窗口，必须计入，否则会严重低估。用一条 **8 格进度条**（带 1/8 块亚像素过渡）+ `用量/总量` 展示，形如 `▋░░░░░░░ 87.5k/1M`；按 `≥80% 红 / ≥50% 黄 / 其余绿` 上色，红了就该 `/compact` 了。
